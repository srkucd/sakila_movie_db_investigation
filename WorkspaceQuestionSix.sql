...
Please go ahead and write a query to compare the payment amounts in each successive month. Repeat this for each of these 10 paying 
customers. Also, it will be tremendously helpful if you can identify the customer name who paid the most difference in terms of payments.
*/
...
t3 AS (SELECT t1.name,
        ...
        ORDER BY t1.name, payment_year, payment_month)

SELECT t3.*,
       CASE
           WHEN t3.lead_dif = (SELECT MAX(t3.lead_dif) FROM t3 ORDER BY MAX(t3.lead_dif) DESC LIMIT 1) THEN 'MAXIMUM'
           ELSE NULL
           END AS is_max					
  FROM t3
 ORDER BY t3.*;
